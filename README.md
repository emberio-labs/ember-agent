...
- Межсессионная память: агент помнит прошлые диалоги — продолжает сессию с тем же
  `session_id` и подтягивает релевантные фрагменты из остальных сессий (recall).
  Хранилище — `FileMemory` из `ember` (JSONL, по файлу на сессию).
...
session_id = "default"         # какая сессия продолжается
...
```bash
ember-agent run --session my-project        # продолжить сессию my-project
...
- сессия — это диалог с одним `session_id`; запуски с тем же `session_id`
  продолжают друг друга;
- из остальных сессий агент подтягивает релевантные фрагменты (кросс-сессионный
  recall) — текущая сессия исключается, её история и так в контексте;
- хранилище — `FileMemory` из `ember`: файл `<session_id>.jsonl` в
...
подключённые инструменты и состояние памяти (`🗂 память: сессия 'default' → .ember/memory`).
