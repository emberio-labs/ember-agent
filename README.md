...
- Красивый вывод в интерактиве: markdown-рендер ответов, приветствие с контекстом
  (провайдер, модель, инструменты), потоковая печать (где доступна), команды сессии
  `/help` и `/reset`, показ процесса вызова инструментов (`🔧`/`✔`/`✖`), Ctrl+C отменяет ответ.
...
В начале сессии печатается контекст (провайдер, модель, подключённые инструменты).
Внутри диалога доступны команды:
